...
// Calcula um cenário completo do Simulador (bruto → líquido) a partir de
// inputs explícitos — fonte única tanto para o modo "Bruto → Líquido" (e,
// por extensão, as taxas usadas em "Líquido → Bruto", que não dependem do
// valor em si) como para cada coluna do modo "Comparar cenários". Nunca há
// duas versões desta conta escritas à parte.
//
// taxaSSPerfil é a taxa de SS do perfil (perfil.taxa_ss); cai para
// TAXA_SS_PADRAO se não vier definida. "retencaoFonte" não entra aqui de
// propósito — não muda o valor do IRS nem do líquido (é sempre devido), só
// muda QUEM já o entregou ao Estado, uma decisão só de rótulo/apresentação,
// tratada na UI. "acumulaOutroTrabalho" é o mesmo campo do Perfil
// (acumula_outro_trabalho) — quem já desconta para a SS por outro emprego
// fica tipicamente isento de descontar também como independente, tal como
// quem está no 1º ano de atividade; são duas situações diferentes que levam
// à mesma isenção, por isso basta uma OR, não uma conta nova.
export function calcularCenarioSimulador({
  valorBruto,
  primeiroAno,
  regiao,
  categoriaCoeficiente,
  taxaSSPerfil,
  acumulaOutroTrabalho = false
}) {
  const taxaIrsContinente = primeiroAno ? 0.25 : 0.115
  const taxaIrs = taxaRetencaoRegional(taxaIrsContinente, regiao)

  const percentagemRendimentoRelevante = percentagemRendimentoRelevanteSS(
    { categoria_coeficiente: categoriaCoeficiente },
    null
  )
  const taxaSSBase = taxaSSPerfil != null ? taxaSSPerfil : TAXA_SS_PADRAO
  const isentoSS = primeiroAno || acumulaOutroTrabalho
  const taxaSS = isentoSS ? 0 : percentagemRendimentoRelevante * taxaSSBase

  const irs = valorBruto * taxaIrs
  const ss = valorBruto * taxaSS
  const liquido = valorBruto - irs - ss

  return { taxaIrs, taxaSS, percentagemRendimentoRelevante, irs, ss, liquido }
}
